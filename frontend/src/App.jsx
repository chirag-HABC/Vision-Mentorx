...
function App() {
  ...
  const speak = useCallback((text) => {
    try {
      ...
      utterance.onstart = () => {
        console.log("Robot starting to speak...");
        setIsSpeaking(true);
      };
  ...
  useEffect(() => {
    const introduceRobot = async () => {
      ...
      const introduction =
        "Hello! I'm your AI Mentor Robot. Welcome! I'm here to have a conversation with you, answer your questions, and help you learn. Please go ahead and ask me anything or just chat with me!";
      setMessages([
        { role: "bot", text: introduction, timestamp: new Date() },
      ]);
  ...
  const sendToAI = async (userText) => {
    ...
    try {
      ...
      const botReply =
        data.reply || "I didn't quite understand that. Could you please repeat?";
      ...
      const errorMsg =
        "Sorry, I couldn't connect to the server. Please make sure the backend is running on port 5000.";
      setMessages((prev) => [
        ...prev,
        { role: "bot", text: errorMsg, timestamp: new Date() },
      ]);
      speak(errorMsg);
    }
  };
  ...
  const startAudioAnalysis = useCallback(async () => {
    ...
    try {
      ...
      setDebugInfo(`Audio init error: ${error.message}`);
    }
  }, []);
  ...
  }, [isWelcomeComplete, startAudioAnalysis]);
  ...
  return (
    ...
    >
      ...
      >
        <h1 style={{ margin: "0", fontSize: "2.5em", marginBottom: "5px" }}>
          VisionMentor X
        </h1>
        <p style={{ margin: "0", fontSize: "0.9em", opacity: 0.9 }}>
          AI Humanoid Robot Conversational Partner
        </p>
      </div>
      ...
      >
        ...
        >
          ...
          >
            <VideoFeed
              onCapture={(dataUrl) => setFaceTextureUrl(dataUrl)}
              onPose={(p) => setPose(p)}
            />
          </div>
        ...
        >
          ...
          >
            <div style={{ display: "flex", gap: "10px", marginBottom: "10px" }}>
              <button
                ...
                style={{
                  ...
                  fontWeight: "bold",
                  cursor:
                    isListening || isSpeaking ? "not-allowed" : "pointer",
            ...
            <small
              style={{ display: "block", marginTop: "8px", opacity: 0.8 }}
            >
